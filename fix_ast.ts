...
for (const sourceFile of project.getSourceFiles()) {
  ...
  for (const sl of sls) {
    ...
    if (/[А-Яа-яЁё]/.test(text)) {
      ...
      // Skip if it's inside an import, or type annotation, or literal type
      if (parent.getKind() === SyntaxKind.ImportDeclaration) continue;
      if (parent.getKind() === SyntaxKind.LiteralType) continue;
      
      // Skip if already inside a `t(...)` call
      if (parent.getKind() === SyntaxKind.CallExpression) {
         const callExpr = parent as any;
         const expression = callExpr.getChildAtIndex(0).getText();
         if (expression.endsWith('t')) continue;
      }
      ...
      if (parent.getKind() === SyntaxKind.JsxAttribute) {
        ...
        if (prop.getNameNode && prop.getNameNode() === sl) {
          ...
          replacements.push({ start, end: parent.getEnd(), text: `[t(${escapedStr}, ${escapedStr})]: ${initText}` });
        } else {
          replacements.push({ start, end, text: `t(${escapedStr}, ${escapedStr})` });
        }
      } else {
  ...
  // Add import and hooks
  // Identify React Components: arrow functions, func expressions, or func declarations starting with Uppercase and returning JSX
  ...
  for (const comp of allComps) {
    ...
    // Check if it's a valid React Component (capitalized name)
    ...
    if (comp.getKind() === SyntaxKind.FunctionDeclaration) {
        ...
        const parentVar = comp.getParent()?.getParent();
  ...
  // Remove duplicates and overlapping replacements (just in case)
...
console.log("AST Replacements Done!");
