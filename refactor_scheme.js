...
let dataContent = `export ` + extractedData.replace(/const /g, 'export const ').replace(/type /g, 'export type ').replace(/interface /g, 'export interface ');

fs.writeFileSync(dataFilePath, dataContent);

const imports = `import { CLR, LEGEND, COLORS, BLOCKS, NODES, ARROWS, CARGO_COLORS, CARGO_LEGEND, FLEET_PLACEHOLDER, LOGISTICS, RADIAL_NODES, RADIAL_FLOWS, TIMELINE_TRIPS, TIMELINE_TRACKS, Cat } from './schemeData';\n\n`;
