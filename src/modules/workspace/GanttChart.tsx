...
const GanttChart: React.FC = () => {
  const tasks = useStore(state => state.tasks);
      ...
      const ganttScale = useStore(state => state.ganttScale);
      const updateTask = useStore(state => state.updateTask);
      const cascadeUpdate = useStore(state => state.cascadeUpdate);
      const openEditTask = useStore(state => state.openEditTask);
      const setGanttScale = useStore(state => state.setGanttScale);
  ...
  // ── Bar geometry ──
  const getBar = (task: { startDate: string; dueDate: string; id?: string; parentId?: string | null }) => {
        ...
        const toolbarContent = (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', background: 'var(--bg-elevated)', borderRadius: 4 }}>
              ...
              <button className="btn btn-ghost" onClick={() => goToToday()} id="gantt-today" style={{ fontSize: 11, height: 24, padding: '0 8px' }}><span className="hide-on-mobile">Сегодня</span><span className="hide-on-desktop">Тк</span></button>
            ...
            >
              <Layers size={12} />
              <span className="hide-on-mobile">Подзадачи</span>
            </button>
