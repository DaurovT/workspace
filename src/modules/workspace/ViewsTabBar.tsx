...
const ViewsTabBar: React.FC = () => {
  const savedViews = useStore(state => state.savedViews);
      const activeSavedViewId = useStore(state => state.activeSavedViewId);
      const setActiveSavedViewId = useStore(state => state.setActiveSavedViewId);
      ...
      const setKanbanGroupBy = useStore(state => state.setKanbanGroupBy);
      const saveView = useStore(state => state.saveView);
      const updateSavedView = useStore(state => state.updateSavedView);
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <div className="view-switcher">
          {[
            ...
            >
              <Icon size={11} /> <span className="hide-on-mobile">{label}</span>
            </button>
          ))}
        </div>
      </div>
