...
const Dashboard: React.FC = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingBottom: 60 }}>
      {/* Приветствие */}
      <div className="mobile-stack-wrap" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div>
      ...
      {/* Главные метрики */}
      <div className="mobile-2-col" style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
        {[
      ...
      <div className="mobile-1-col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {/* Прогресс проектов */}
            ...
            {teamWorkload.map(({ user, total, done }) => {
              ...
              return (
                <div key={user.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div className="avatar" style={{ width: 22, height: 22, fontSize: 8, background: user?.color ?? '#888', flexShrink: 0 }}>{user?.avatar ?? '?'}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                      <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.name?.split(' ')[0] ?? 'N/A'}</span>
                      <span style={{ fontSize: 10, color: 'var(--text-muted)', flexShrink: 0 }}>{total} активных</span>
                    </div>
                    <div className="progress-bar">
                      <div className="progress-fill" style={{ width: `${pct}%`, background: user?.color ?? '#888' }} />
                    </div>
                  </div>
                </div>
              );
