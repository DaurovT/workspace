...
import { Filter, ArrowUpDown, CheckCircle2, Link as LinkIcon, Clock, ChevronRight } from 'lucide-react';
...
const STATUS_COLORS: Record<string, { bg: string, text: string, dot: string }> = {
  todo: { bg: 'rgba(148,163,184,0.15)', text: 'var(--text-secondary)', dot: 'var(--text-muted)' },
  in_progress: { bg: 'rgba(59,130,246,0.15)', text: '#3b82f6', dot: '#3b82f6' },
  review: { bg: 'rgba(245,158,11,0.15)', text: '#f59e0b', dot: '#f59e0b' },
  done: { bg: 'rgba(16,185,129,0.15)', text: '#10b981', dot: '#10b981' }
};
...
const StatusBadge: React.FC<{ status: StatusId }> = ({ status }) => {
  ...
  return (
    <span style={{ 
      ...
      background: color.bg, padding: '4px 10px', borderRadius: 12,
      border: `1px solid ${color.bg.replace('0.15', '0.3')}`
    }}>
      <span style={{ width: 6, height: 6, borderRadius: '50%', background: color.dot, boxShadow: `0 0 4px ${color.dot}` }} />
      <span style={{ fontSize: 11, fontWeight: 600, color: color.text, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
...
const TaskList: React.FC = () => {
  const tasks = useStore(state => state.tasks);
      ...
      const filterAssignee = useStore(state => state.filterAssignee);
      ...
      const setFilterAssignee = useStore(state => state.setFilterAssignee);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 16 }}>
      {/* Статистика — горизонтальная лента */}
      <div className="stats-strip">
        {[
          ...
          <div key={label} className="stat-chip">
            <div className="stat-chip-dot" style={{ background: color }} />
      ...
      <div className="filters-row" style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        <span className="hide-on-mobile" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-muted)', fontWeight: 600 }}>
          <Filter size={13} /> Фильтры:
        </span>
        <span className="show-on-mobile hide-on-desktop" style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'var(--text-muted)' }}>
          <Filter size={16} />
        </span>
        <select
      ...
      {/* Сетка задач (премиальный вид) */}
      <div style={{
        flex: 1,
        display: 'flex', flexDirection: 'column',
        overflow: 'hidden',
        padding: '0 8px' // slight padding for box-shadows to not clip
      }}>
        <div className="table-responsive" style={{ flex: 1, overflowX: 'auto', display: 'flex', flexDirection: 'column' }}>
          <div className="task-list-container" style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
            {/* Table header */}
            <div className="task-list-header">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }} className="task-cell-cb">
            <input
              type="checkbox"
              id="select-all-tasks"
          ...
          <div className="task-cell-done" />
          <div className="task-cell-title" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            Задача <ArrowUpDown size={10} style={{ opacity: 0.5 }} />
          </div>
          <div className="task-cell-status">Статус</div>
          <div className="task-cell-priority">Приоритет</div>
          <div className="task-cell-assignee">Исполнитель</div>
          <div className="task-cell-due">Срок</div>
        </div>
          ...
          {projectTasks.length === 0 ? (
            ...
            projectTasks.map(task => {
              ...
              return (
                <div
                  key={task.id}
                  className={`task-row ${task.priority} ${selectedTaskIds.includes(task.id) ? 'selected' : ''}`}
                  onClick={() => openEditTask(task.id)}
                  id={`task-row-${task.id}`}
                >
                  {/* Done toggle — круглая кнопка слева (главное действие) */}
                  <div
                    className={`task-done-btn ${isDone ? 'done' : ''}`}
                    onClick={(e) => toggleDone(task.id, task.status, e)}
                    title={isDone ? 'Отменить выполнение' : 'Отметить выполненным'}
                  >
                    {isDone && (
                  ...
                  {/* Select checkbox — справа, только на десктопе */}
                  <div className="task-cell-cb task-select-cb" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => e.stopPropagation()}>
                    <input 
                      type="checkbox" 
                  ...
                  <div className="task-cell-title" style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    <span className={`task-title ${isDone ? 'done' : ''}`} style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {task.title}
                    </span>
                    {/* Inline chips */}
                    <div className="task-inline-chips" style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                  ...
                  {/* Meta row — wraps on mobile into single line */}
                  <div className="task-meta-row">
                    ...
                    {/* Assignee */}
                    <div className="task-cell-assignee" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {assignee ? (
                        <>
                    ...
                    {/* Due date */}
                    <div className="task-cell-due">
                      {task.dueDate ? (
                        <span style={getDueDateStyle(task.dueDate, task.status)}>
      ...
      {selectedTaskIds.length > 0 && (
        <div className="mobile-stack-wrap" style={{
          ...
          animation: 'slideUp 0.2s ease', width: '90%', maxWidth: 500,
        }}>
          ...
          <div className="mobile-stack-wrap" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <select
