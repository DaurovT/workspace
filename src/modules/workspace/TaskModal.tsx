...
const TaskModal: React.FC = () => {
  const isTaskModalOpen = useStore(state => state.isTaskModalOpen);
      const editingTaskId = useStore(state => state.editingTaskId);
      const tasks = useStore(state => state.tasks);
      const users = useStore(state => state.users);
      const loadCommentsForTask = useStore(state => state.loadCommentsForTask);

  useEffect(() => {
    if (isTaskModalOpen && editingTaskId) {
      loadCommentsForTask(editingTaskId);
    }
  }, [isTaskModalOpen, editingTaskId, loadCommentsForTask]);
  ...
  const isGhostAssignee = assigneeId && !users.find(u => u.id === assigneeId);
      ...
      <div
        ...
        onKeyDown={e => {
          ...
          if (e.key === 'Enter' && !e.shiftKey) {
            ...
            if (isGhostAssignee) return; // block saving if ghost
            e.preventDefault();
            handleSave();
          }
          // Esc — close
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {isGhostAssignee && (
               <span style={{ fontSize: 11, color: 'var(--color-danger)', display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(239,68,68,0.1)', padding: '2px 8px', borderRadius: 12 }}>
                 <AlertTriangle size={11} /> Назначьте нового исполнителя
               </span>
            )}
          </div>
        ...
        {/* ── Body ───────────────────────────────────────────────────────────── */}
        <div className="task-modal-body" style={{ display: 'flex', flex: 1, overflow: 'hidden', minHeight: 0 }}>

          {/* Left — main content */}
          <div className="task-modal-main" style={{
            ...
            <textarea
              id="task-title"
              className="form-control task-modal-title"
              style={{
                fontSize: 24, 
                fontWeight: 700, 
                padding: '10px 14px', 
                    ...
                    return (
                      ...
                      >
                        ...
                        {stAssignee ? (
                          ...
                          <div className="avatar" style={{ width: 20, height: 20, fontSize: 10, background: 'var(--bg-elevated)', border: '1px solid var(--color-danger)', color: 'var(--color-danger)', flexShrink: 0 }}>
                            👻
                          </div>
                        ) : null}
                      </div>
                    ...
                    {(existingTask.dependencies || []).map((dep, idx) => {
                      ...
                      return (
                        <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', background: 'var(--bg-elevated)', borderRadius: 8, border: '1px solid var(--border-subtle)' }}>
                          ...
                          {(dep.lag ?? 0) !== 0 && <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>{(dep.lag ?? 0) > 0 ? `+${dep.lag}д` : `${dep.lag ?? 0}д`}</span>}
                  ...
                  {visibleComments.map(c => {
                    ...
                    return (
                      <div key={c.id} style={{ display: 'flex', gap: 10 }}>
                        <div className="avatar" style={{ width: 26, height: 26, fontSize: 9, background: author?.color ?? 'var(--bg-elevated)', border: author ? 'none' : '1px solid var(--border-subtle)', flexShrink: 0 }}>
                          {author?.avatar ?? '👻'}
                        </div>
                        <div style={{ flex: 1, background: 'var(--bg-elevated)', borderRadius: 8, padding: '8px 12px', border: '1px solid var(--border-subtle)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                            <span style={{ fontSize: 11, fontWeight: 600, color: author ? 'var(--text-primary)' : 'var(--text-muted)' }}>{author?.name.split(' ')[0] ?? 'Удаленный'}</span>
          ...
          <div className="task-modal-sidebar" style={{
            ...
            <PropRow icon={<User size={11} />} label="Исполнитель">
              {assignee ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div className="avatar" style={{ width: 20, height: 20, fontSize: 11, background: 'rgba(239,68,68,0.1)', color: 'var(--color-danger)', border: '1px solid var(--color-danger)', flexShrink: 0 }}>👻</div>
                  <select
                    id="task-assignee"
                    className="form-control select-control"
                    style={{ height: 30, fontSize: 11, flex: 1, borderColor: 'var(--color-danger)', color: 'var(--color-danger)', fontWeight: 600 }}
                    value={assigneeId}
                    onChange={e => setAssigneeId(e.target.value)}
                  >
                    <option value={assigneeId} disabled>Удаленный пользователь</option>
                    <option value="">Не назначен</option>
                    {users.map(u => <option key={u.id} value={u.id} style={{ color: 'var(--text-primary)' }}>{u.name}</option>)}
                  </select>
                </div>
              ) : (
                <select
                  ...
                  {addH && (
                    <div style={{ display: 'flex', gap: 6 }}>
                      <input 
                        ...
                        onKeyDown={e => {
                          if (e.key === 'Enter') {
                            useStore.getState().logTime(existingTask.id, Number(addH), logDate, logNotes);
                            setAddH(''); setLogNotes('');
                          }
                        }}
                      />
        ...
        <div className="task-modal-footer" style={{
          ...
          <button 
            ...
            onClick={handleSave} 
            disabled={!!isGhostAssignee}
            title={isGhostAssignee ? 'Необходимо выбрать нового исполнителя вместо удаленного' : ''}
            id="btn-save-task"
          >
