      ...
      <div className="premium-users-grid">
        {filtered.map((user) => {
          ...
          return (
            ...
            >
              {/* Avatar */}
              <div className="premium-card-avatar-wrapper">
                <div className="avatar" style={{ background: user.color }}>{user.avatar}</div>
                <div 
                  className="premium-card-status-dot" 
                  style={{ background: user.isActive ? 'var(--color-success)' : 'var(--text-muted)' }} 
                />
              </div>

              {/* Content */}
              <div className="premium-card-content">
                <div className="premium-card-name-row">
                  <div className="premium-card-name">{user.name}</div>
                  <RoleBadge role={user.role} />
                </div>
                <div className="premium-card-email">{user.email || '—'}</div>
                
                <div className="premium-card-job">
                  {user.jobTitle || 'Нет должности'}
                </div>

                {/* Projects */}
                <div className="premium-card-projects">
              ...
              {/* Action Button */}
              <button 
                className="btn btn-icon premium-card-action"
...
const TeamPage: React.FC = () => {
  ...
  return (
    <div className="team-page-container">
      {/* Header */}
      <div className="mobile-stack-wrap" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, gap: 12 }}>
        <div>
      ...
      {isOwnerOrAdmin && (
        <div className="team-stats-row">
          {[
            { label: 'Участников', value: stats.total, color: '#6366f1', icon: <Users size={15} /> },
            ...
            <div key={s.label} className="team-stat-card" style={{
              ...
              {/* Subtle side glow */}
              <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 3, background: s.color, opacity: 0.8 }} />
              
              <div style={{ width: 42, height: 42, borderRadius: 12, background: `linear-gradient(135deg, ${s.color}20, ${s.color}05)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: s.color, flexShrink: 0, border: `1px solid ${s.color}15` }}>
                {s.icon}
              </div>
              <div>
                <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)', lineHeight: 1 }}>{s.value}</div>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-secondary)', marginTop: 4 }}>{s.label}</div>
              </div>
            </div>
          ))}
        </div>
