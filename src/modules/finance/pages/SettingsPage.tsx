    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: 'var(--bg-base)' }}>
      ...
      <div style={{ width: 220, flexShrink: 0, background: 'var(--bg-surface)', borderRight: '1px solid var(--border-subtle)', padding: '12px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        <div style={{ height: 44, display: 'flex', alignItems: 'center', padding: '0 10px', marginBottom: 8 }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Настройки</span>
        </div>
        ...
        <button 
          ...
          <Settings size={18} /> Общие настройки
        </button>
        ...
        <button 
          ...
          <Users size={18} /> Пользователи (Команда)
        </button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
        ...
        {activeTab === 'general' && (
          <div style={{ maxWidth: 800 }}>
            <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 20 }}>Общие настройки</span>
            
            <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 10, padding: 20, marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, color: 'var(--text-primary)', margin: '0 0 16px' }}>Основная информация</h3>
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8 }}>Название бизнеса</label>
                <input 
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8 }}>Основная валюта аккаунта</label>
                <select 
                ...
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 8 }}>Все сводные данные в отчетах (P&L, Cashflow) будут конвертироваться в эту валюту по курсу ЦБ.</p>
              </div>
            </div>

            <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 12, padding: 24 }}>
              <h3 style={{ fontSize: 16, color: 'var(--text-primary)', margin: '0 0 16px' }}>Настройки управленческого учета (P&L)</h3>
              <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>Классификация расходов</label>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: 16, border: settings.pnlMode === 'direct_indirect' ? '1px solid rgba(59, 130, 246, 0.5)' : '1px solid var(--border-default)', background: settings.pnlMode === 'direct_indirect' ? 'rgba(59, 130, 246, 0.05)' : 'transparent', borderRadius: 8, cursor: 'pointer' }}>
                  ...
                  <div>
                    <div style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 14, marginBottom: 4 }}>Прямые и Косвенные (Рекомендуется для услуг)</div>
                    <div style={{ color: 'var(--text-secondary)', fontSize: 13 }}>Прямые расходы зависят от проекта (ЗП монтажникам), косвенные — общие (Аренда офиса).</div>
                  </div>
                </label>
                ...
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: 16, border: settings.pnlMode === 'fixed_variable' ? '1px solid rgba(59, 130, 246, 0.5)' : '1px solid var(--border-default)', background: settings.pnlMode === 'fixed_variable' ? 'rgba(59, 130, 246, 0.05)' : 'transparent', borderRadius: 8, cursor: 'pointer' }}>
                  ...
                  <div>
                    <div style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 14, marginBottom: 4 }}>Постоянные и Переменные (Для торговли)</div>
                    <div style={{ color: 'var(--text-secondary)', fontSize: 13 }}>Переменные растут вместе с продажами (Себестоимость товара), постоянные фиксы (Оклады).</div>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div style={{ background: 'var(--bg-surface)', border: '1px solid #f59e0b', borderRadius: 12, padding: 24, marginTop: 16 }}>
              <h3 style={{ fontSize: 16, color: '#f59e0b', margin: '0 0 16px' }}>Закрытие финансового периода</h3>
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>Защитите прошлые периоды от редактирования. Любые транзакции до или в указанную дату будут заблокированы.</p>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', fontWeight: 600 }}>Дата блокировки (Lock Date)</label>
                <input 
                  type="date"
        ...
        {activeTab === 'users' && (
          <div style={{ maxWidth: 900 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', margin: 0 , whiteSpace: 'nowrap' }}>Команда</span>
            ...
            <div style={{ background: 'var(--bg-surface)', borderRadius: 12, border: '1px solid var(--border-default)', overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{ background: 'var(--bg-surface)' }}>
                  <tr>
                    <th style={{ padding: '8px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Сотрудник</th>
                    <th style={{ padding: '8px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Роль</th>
                    <th style={{ padding: '8px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Статус</th>
                    <th style={{ padding: '8px 14px', textAlign: 'center', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Действия</th>
                  </tr>
                </thead>
                <tbody>
                  {users.map(u => (
                    <tr key={u.id} style={{ borderTop: '1px solid var(--border-subtle)' }}>
                      ...
                      <td style={{ padding: 16 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: u.status === 'Подтвержден' ? '#10b981' : 'var(--text-muted)', fontSize: 13 }}>
                          {u.status === 'Подтвержден' ? <CheckCircle size={14} /> : null} {u.status}
                        </div>
                      </td>
                      <td style={{ padding: '8px 14px', textAlign: 'center' }}>
                        {u.role !== 'Владелец' ? (
                          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                            <button onClick={() => handleOpenPermissions(u.id, u.permissions)} style={{ background: 'transparent', border: 'none', color: '#3b82f6', fontSize: 13, cursor: 'pointer' }}>Настройки прав</button>
                            <button onClick={() => deleteUser(u.id)} style={{ background: 'transparent', border: 'none', color: '#f43f5e', fontSize: 13, cursor: 'pointer' }}>Отозвать</button>
                          </div>
                        ) : '—'}
                      </td>
                    </tr>
        ...
        {activeTab === 'import_rules' && (
          <div style={{ maxWidth: 800 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <div>
                <h2 style={{ fontSize: 20, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 4 }}>Правила авто-разноски (Smart Import)</h2>
                <div style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Система автоматически применяет эти правила (сверху вниз) при загрузке банковской выписки или ручном вводе.</div>
              </div>
            ...
            <div style={{ display: 'table', width: '100%', borderCollapse: 'collapse' }}>
              <div style={{ display: 'table-row', borderBottom: '1px solid var(--border-subtle)', color: 'var(--text-muted)', fontSize: 12, fontWeight: 600 }}>
                <div style={{ display: 'table-cell', padding: '12px 16px', width: 40 }}>Статус</div>
                <div style={{ display: 'table-cell', padding: '12px 16px' }}>Название и Условие</div>
                <div style={{ display: 'table-cell', padding: '12px 16px' }}>Действие системы</div>
                ...
                <div key={rule.id} style={{ display: 'table-row', borderBottom: '1px solid var(--border-subtle)' }}>
                  ...
                  <div style={{ display: 'table-cell', padding: '16px', verticalAlign: 'middle' }}>
                    ...
                    <div style={{ color: 'var(--text-secondary)', fontSize: 13 }}>
                      Eсли <strong style={{ color: '#38bdf8' }}>{rule.conditionField}</strong> {rule.conditionOperator === 'contains' ? 'содержит' : 'равно'} "{rule.conditionValue}"
                    </div>
                  </div>
                  <div style={{ display: 'table-cell', padding: '16px', verticalAlign: 'middle' }}>
                    <div style={{ color: 'var(--text-primary)', fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
                      <CheckCircle size={14} color="#10b981" /> Заменить на: {rule.actionValue}
                    </div>
                  </div>
                  <div style={{ display: 'table-cell', padding: '16px', verticalAlign: 'middle' }}>
              ...
              {importRules.length === 0 && (
                <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>Нет правил автоматизации.</div>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'audit' && (
          <div style={{ maxWidth: 1000 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              <div>
                <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 8px' }}>Журнал аудита</span>
                <div style={{ color: 'var(--text-secondary)', fontSize: 14 }}>Хронология действий и изменений в аккаунте. Включен режим Strict Compliance.</div>
              </div>
              <button 
                onClick={() => {
                  const rows = [['Дата', 'Сотрудник', 'Действие', 'Сущность', 'Детали'],
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead style={{ background: 'var(--bg-surface)' }}>
                  <tr style={{ color: 'var(--text-muted)', fontSize: 12 }}>
                    <th style={{ padding: '8px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Дата и Время</th>
                    <th style={{ padding: '8px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Пользователь</th>
                    <th style={{ padding: '8px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Событие</th>
                    <th style={{ padding: '8px 14px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border-subtle)', whiteSpace: 'nowrap' }}>Детали</th>
                  </tr>
                </thead>
                <tbody>
                  {auditLogs.map(log => (
                  ...
                  {auditLogs.length === 0 && (
                    <tr><td colSpan={4} style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>Нет записей аудита</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: 'var(--bg-surface)', width: 400, borderRadius: 12, border: '1px solid var(--border-subtle)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-subtle)' }}>
              <h2 style={{ margin: 0, fontSize: 18, color: 'var(--text-primary)' }}>Пригласить пользователя</h2>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8 }}>ФИО (Для отображения)</label>
                <input id="settings-field-3" name="settings-field-3" value={newName} onChange={e => setNewName(e.target.value)} type="text" placeholder="Иван Иванов" style={{ width: '100%', padding: '10px 14px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 8, color: 'var(--text-primary)', fontSize: 14 }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8 }}>Роль в системе</label>
                <select id="settings-select-9" name="settings-select-9" value={newRole} onChange={e => setNewRole(e.target.value as any)} style={{ width: '100%', padding: '10px 14px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 8, color: 'var(--text-primary)', fontSize: 14 }}>
                  ...
                  <option value="Менеджер">Менеджер (Чтение и запись)</option>
                  <option value="Гость">Гость (Только чтение)</option>
                </select>
              </div>
            </div>
            <div style={{ padding: '16px 24px', background: 'var(--bg-surface)', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <button onClick={() => setInviteModalOpen(false)} style={{ padding: '10px 16px', background: 'transparent', border: 'none', color: 'var(--text-primary)', fontWeight: 600, cursor: 'pointer' }}>Отмена</button>
              <button 
                onClick={() => {
                  if (newEmail) {
      ...
      {isPermissionsModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: 'var(--bg-surface)', width: 500, borderRadius: 12, border: '1px solid var(--border-default)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-subtle)' }}>
              <h2 style={{ margin: 0, fontSize: 18, color: 'var(--text-primary)' }}>Управление Правами Доступа</h2>
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, maxHeight: 400, overflowY: 'auto' }}>
              <p style={{ color: 'var(--text-secondary)', fontSize: 13, margin: '0 0 8px' }}>Отметьте разделы, к которым данный сотрудник будет иметь доступ. Разделы без галочек будут скрыты из левого меню.</p>
            ...
            <div style={{ padding: '16px 24px', background: 'var(--bg-surface)', display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <button onClick={() => setPermissionsModalOpen(false)} style={{ padding: '10px 16px', background: 'transparent', border: 'none', color: 'var(--text-primary)', fontWeight: 600, cursor: 'pointer' }}>Отмена</button>
              <button 
                onClick={() => {
      ...
      {/* Rule Creation Modal */}
      <Modal isOpen={isRuleModalOpen} onClose={() => setIsRuleModalOpen(false)} title="Создать правило авто-разноски" width="500px">
        <form onSubmit={handleCreateRule} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 6 }}>Название правила</label>
          ...
          <div style={{ background: 'rgba(56, 189, 248, 0.05)', padding: 16, borderRadius: 8, border: '1px dashed rgba(56, 189, 248, 0.3)' }}>
            <div style={{ marginBottom: 12, fontWeight: 600, color: '#38bdf8' }}>ШАГ 1: Если операция...</div>
            <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 6 }}>Назначение содержит слово/маску:</label>
            <input id="settings-text-6" name="settings-text-6" type="text" value={newRuleConditionVal} onChange={e => setNewRuleConditionVal(e.target.value)} required placeholder="Например: Яндекс.Маркет" style={{ width: '100%', padding: '10px 14px', background: 'rgba(0,0,0,0.4)', border: '1px solid var(--border-default)', borderRadius: 8, color: 'var(--text-primary)' }} />
          </div>
          <div style={{ background: 'rgba(16, 185, 129, 0.05)', padding: 16, borderRadius: 8, border: '1px dashed rgba(16, 185, 129, 0.3)' }}>
            <div style={{ marginBottom: 12, fontWeight: 600, color: '#10b981' }}>ШАГ 2: Выполнить действие</div>
            <label style={{ display: 'block', fontSize: 13, color: 'var(--text-secondary)', marginBottom: 6 }}>Подставить системную статью (ID):</label>
            <input id="settings-text-7" name="settings-text-7" type="text" value={newRuleActionVal} onChange={e => setNewRuleActionVal(e.target.value)} required placeholder="ID статьи" style={{ width: '100%', padding: '10px 14px', background: 'rgba(0,0,0,0.4)', border: '1px solid var(--border-default)', borderRadius: 8, color: 'var(--text-primary)' }} />
          </div>
