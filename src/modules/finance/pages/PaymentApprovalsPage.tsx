    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden', background: 'var(--bg-base)', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ width: 340, background: 'var(--bg-surface)', borderRight: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '20px 20px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 16, fontWeight: 600, color: 'var(--text-primary)', margin: 0, letterSpacing: -0.2 }}>Заявки</span>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-subtle)', padding: '0 20px', gap: 16 }}>
          {[
            ...
            { id: 'approved', label: 'Готовы', count: approved.length },
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  {prq.status === 'pending_manager' && <span style={{ color: 'var(--text-muted)', fontSize: 10, fontWeight: 600, textTransform: 'uppercase' }}>РУК-ЛЬ</span>}
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                {new Intl.NumberFormat('ru-RU').format(prq.amount)} сум
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-base)' }}>
        {!selectedReq ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>
            <span style={{ fontSize: 14 }}>Выберите заявку из списка слева</span>
          </div>
        ) : (
          <>
            {/* Header Toolbar */}
            <div style={{ padding: '24px 32px 0', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {(selectedReq.status === 'pending_ceo' || selectedReq.status === 'pending_manager') && (
                  <>
                    <button onClick={() => updatePaymentRequestStatus(selectedReq.id, 'rejected')} style={{ background: 'transparent', color: 'var(--text-primary)', border: '1px solid var(--border-subtle)', padding: '0 16px', height: 28, borderRadius: 6, fontSize: 12, fontWeight: 500, cursor: 'pointer', transition: 'all 0.1s' }}>Отклонить</button>
                ...
                {selectedReq.status === 'approved' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                    <select
                      ...
                      {accounts.map(a => (
                        <option key={a.id} value={a.id}>{a.name} ({new Intl.NumberFormat('ru-RU').format(a.balance)} сум)</option>
                      ))}
                    </select>
                    <button
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '40px 32px' }}>
              <div style={{ maxWidth: 640 }}>
                ...
                <div style={{ fontSize: 32, fontWeight: 500, color: 'var(--text-primary)', margin: '0 0 32px', letterSpacing: -0.5 }}>{new Intl.NumberFormat('ru-RU').format(selectedReq.amount)} сум</div>
                ...
                <div style={{ borderRadius: 8, overflow: 'hidden', marginBottom: 40, background: 'rgba(0,0,0,0.02)' }}>
                  ...
                  <div style={{ display: 'flex', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                    <div style={{ width: 140, padding: '16px 20px', color: 'var(--text-muted)', fontSize: 13, fontWeight: 400 }}>Получатель</div>
                    <div style={{ flex: 1, padding: '16px 20px', color: 'var(--text-primary)', fontSize: 13 }}>{reqContractor?.name || '—'}</div>
                  </div>
                  
                  <div style={{ display: 'flex', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                    <div style={{ width: 140, padding: '16px 20px', color: 'var(--text-muted)', fontSize: 13, fontWeight: 400 }}>Статья расходов</div>
                  ...
                  <div style={{ display: 'flex' }}>
                    <div style={{ width: 140, padding: '16px 20px', color: 'var(--text-muted)', fontSize: 13, fontWeight: 400 }}>Желаемая дата</div>
                ...
                <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 20px' }}>Активность</h3>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20, position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, zIndex: 1 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 2 }}>Заявка создана</div>
                  ...
                  <div style={{ display: 'flex', gap: 16, zIndex: 1 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 2 }}>Согласование руководством</div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, zIndex: 1, opacity: selectedReq.status === 'paid' ? 1 : 0.4 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 2 }}>Исполнение платежа</div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
