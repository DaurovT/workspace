      ...
      {isSidebarOpen && (
        <div style={{ width: 200, flexShrink: 0, borderRight: '1px solid var(--border-subtle)', background: 'var(--bg-surface)', display: 'flex', flexDirection: 'column' }}>
          <div style={{ height: 44, padding: '0 16px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Filter size={13} color="var(--text-muted)" /><span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>Фильтры</span></div>
          ...
          <div style={{ flex: 1, padding: 12, overflowY: 'auto' }}>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', marginBottom: 8 }}>Тип организации</div>
            {TYPE_FILTER.map(t => (
        ...
        <div style={{ width: 280, flexShrink: 0, borderRight: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', background: 'var(--bg-surface)' }}>
          ...
          <div style={{ height: 44, padding: '0 12px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0, flex: 1, overflow: 'hidden' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>Мои юрлица</span>
            ...
            <button onClick={() => setCreateOpen(true)} className="header-btn header-btn-primary" style={{ flexShrink: 0, gap: 4, fontSize: 12 }}><Plus size={11} /> Добавить</button>
          </div>

          {/* List */}
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {filtered.length === 0 && <div style={{ padding: 24, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>Нет организаций</div>}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 3, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>ИНН {e.inn}</span>
                      ...
                      {e.isMain && <span style={{ fontSize: 10, fontWeight: 600, color: '#10b981', background: 'rgba(16,185,129,0.1)', padding: '0 5px', borderRadius: 4 }}>Основное</span>}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {selected ? (
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Карточка организации</div>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
                  ...
                  {selected.isMain && <span style={{ fontSize: 11, fontWeight: 600, color: '#10b981', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', padding: '2px 8px', borderRadius: 4 }}>✓ Основное юрлицо</span>}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                }} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '0 12px', height: 30, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 6, fontSize: 12, color: '#10b981', cursor: 'pointer' }}><CheckCircle2 size={12} /> Сделать основным</button>}
                <button onClick={() => { setEditMode(true); setCreateOpen(true); setF({ ...selected }); }} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '0 12px', height: 30, background: 'var(--bg-elevated)', border: '1px solid var(--border-default)', borderRadius: 6, fontSize: 12, color: 'var(--text-secondary)', cursor: 'pointer' }}><Edit2 size={12} /> Изменить</button>
                {!selected.isMain && <button onClick={() => handleDelete(selected.id)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '0 12px', height: 30, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 6, fontSize: 12, color: '#ef4444', cursor: 'pointer' }}><Trash2 size={12} /> Удалить</button>}
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 10, padding: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}><FileText size={13} /> Регистрация</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <Field label="Тип / ОПФ" value={selected.type} />
                  <Field label="ИНН / ПИНФЛ" value={selected.inn} mono />
                  {selected.kpp && <Field label="КПП" value={selected.kpp} mono />}
                  <Field label="ОГРН / ОГРНИП" value={selected.ogrn} mono />
                </div>
              </div>

              {/* Контакты */}
              <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 10, padding: 16 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}><MapPin size={13} /> Адреса и контакты</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <Field label="Юридический адрес" value={selected.legalAddress} />
                  <Field label="Фактический адрес" value={selected.actualAddress} />
              ...
              <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 10, padding: 16, gridColumn: '1 / -1' }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}><CreditCard size={13} /> Банковские реквизиты</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                  <Field label="Банк" value={selected.bankName} />
                  <Field label="МФО" value={selected.bankMfo} mono />
                  <Field label="Расчётный счёт" value={selected.bankAccount} mono />
                </div>
              </div>
            </div>
          </div>
        ) : (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: 13 }}>Выберите организацию из списка</div>
        )}
      </div>
          ...
          <div style={{ position: 'relative', width: 520, maxHeight: '88vh', background: 'var(--bg-surface)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div><label style={sLbl}>Тип (ОПФ)</label><select value={f.type} onChange={e => setFld('type', e.target.value)} style={{ ...sInp, height: 30 }}>{['МЧЖ','АЖ','ЯТТ','ДК','ХК'].map(t => <option key={t}>{t}</option>)}</select></div>
                <div><label style={sLbl}>Налогообложение</label><select value={f.vatMode} onChange={e => setFld('vatMode', e.target.value)} style={{ ...sInp, height: 30 }}>{['ОСН','УСН 6%','УСН 15%','ПСН','НПД','ЕСХН'].map(v => <option key={v}>{v}</option>)}</select></div>
              </div>
              <div><label style={sLbl}>Краткое наименование *</label><input value={f.shortName || ''} onChange={e => setFld('shortName', e.target.value)} placeholder='МЧЖ «Компания»' style={sInp} /></div>
              <div><label style={sLbl}>Полное наименование</label><input value={f.name || ''} onChange={e => setFld('name', e.target.value)} placeholder='Масъулияти чекланган жамият...' style={sInp} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div><label style={sLbl}>ИНН / ПИНФЛ *</label><input value={f.inn || ''} onChange={e => setFld('inn', e.target.value)} placeholder='123456789' style={{ ...sInp, fontFamily: 'monospace' }} /></div>
                <div><label style={sLbl}>ОГРН / ОГРНИП</label><input value={f.ogrn || ''} onChange={e => setFld('ogrn', e.target.value)} style={{ ...sInp, fontFamily: 'monospace' }} /></div>
              </div>
              <div><label style={sLbl}>Юридический адрес</label><input value={f.legalAddress || ''} onChange={e => setFld('legalAddress', e.target.value)} style={sInp} /></div>
              <div><label style={sLbl}>Телефон</label><input value={f.phone || ''} onChange={e => setFld('phone', e.target.value)} placeholder='+998 71 000-00-00' style={sInp} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 12 }}>
                <div><label style={sLbl}>Банк</label><input value={f.bankName || ''} onChange={e => setFld('bankName', e.target.value)} style={sInp} /></div>
                <div><label style={sLbl}>МФО (5 цифр)</label><input value={f.bankMfo || ''} onChange={e => setFld('bankMfo', e.target.value)} maxLength={5} style={{ ...sInp, fontFamily: 'monospace' }} /></div>
              </div>
              <div><label style={sLbl}>Расчётный счёт (20 цифр)</label><input value={f.bankAccount || ''} onChange={e => setFld('bankAccount', e.target.value)} maxLength={20} style={{ ...sInp, fontFamily: 'monospace' }} /></div>
            </div>
            <div style={{ padding: '12px 20px', background: 'var(--bg-base)', borderTop: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => setCreateOpen(false)} style={{ padding: '0 14px', height: 30, borderRadius: 6, background: 'transparent', border: '1px solid var(--border-default)', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 12 }}>Отмена</button>
