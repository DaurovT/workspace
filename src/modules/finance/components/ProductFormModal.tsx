...
export const ProductFormModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Новая позиция" width="450px">
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        
        <div style={{ display: 'flex', gap: 16, marginBottom: 8 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-primary)', fontSize: 14, cursor: 'pointer' }}>
            <input id="productformmodal-radio-1" name="productformmodal-radio-1" type="radio" checked={type === 'Товар'} onChange={() => setType('Товар')} style={{ accentColor: '#38bdf8' }} /> Товар
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text-primary)', fontSize: 14, cursor: 'pointer' }}>
            <input id="productformmodal-radio-2" name="productformmodal-radio-2" type="radio" checked={type === 'Услуга'} onChange={() => setType('Услуга')} style={{ accentColor: '#38bdf8' }} /> Услуга
          </label>
        </div>

        <div>
          <label style={labelStyle}>Наименование <span style={{ color: '#38bdf8' }}>*</span></label>
          <input id="productformmodal-text-3" name="productformmodal-text-3" type="text" value={name} onChange={e => setName(e.target.value)} required placeholder="Например: Ноутбук Lenovo" style={inputStyle} />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div>
            <label style={labelStyle}>Ед. измерения</label>
            <input id="productformmodal-text-5" name="productformmodal-text-5" type="text" value={unit} onChange={e => setUnit(e.target.value)} placeholder="шт / час / кг" style={inputStyle} />
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div>
            <label style={labelStyle}>Цена (Без НДС) <span style={{ color: '#38bdf8' }}>*</span></label>
        ...
        {type === 'Товар' && (
          <div>
            <label style={labelStyle}>Начальный остаток на складе</label>
        ...
        {price && (
          <div style={{ marginTop: 8, padding: 12, background: 'rgba(56, 189, 248, 0.1)', borderRadius: 8, border: '1px dashed rgba(56, 189, 248, 0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 13, color: 'rgba(56, 189, 248, 0.8)' }}>Итоговая цена с НДС:</span>
