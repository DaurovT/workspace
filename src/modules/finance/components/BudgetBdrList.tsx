    ...
    <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.4)',backdropFilter:'blur(4px)'}}>
      <div style={{background:'var(--bg-surface)',borderRadius:12,width:480,boxShadow:'0 20px 60px rgba(0,0,0,0.3)',border:'1px solid var(--border-subtle)'}}>
        ...
        <div style={{padding:'20px 24px 16px',borderBottom:'1px solid var(--border-subtle)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <h2 style={{margin:0,fontSize:16,fontWeight:700,color:'var(--text-primary)'}}>{t("Новый бюджет доходов и расходов", "Новый бюджет доходов и расходов")}</h2>
        ...
        <form onSubmit={handleSubmit} style={{padding:'20px 24px',display:'flex',flexDirection:'column',gap:14}}>
          <div style={row}>
            ...
            <input style={inp} placeholder={t("Например: БДР 2026 Основной", "Например: БДР 2026 Основной")} value={form.name} onChange={e=>upd('name',e.target.value)} required/>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div style={row}>
              <span style={label}>{t("Тип", "Тип")}</span>
              <select style={{...inp,appearance:'none',paddingRight:28}} value={form.type} onChange={e=>upd('type',e.target.value as 'БДР'|'БДДС')}>
                <option value={t("БДР", "БДР")}>{t("БДР", "БДР")}</option>
                <option value={t("БДДС", "БДДС")}>{t("БДДС", "БДДС")}</option>
              </select>
            </div>
            <div style={row}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div className="finance-page-header" style={{ height: 44, padding: '0 24px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: 'var(--bg-surface)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', letterSpacing: -0.01 }}>{t("БДР", "БДР")}</span>
              <span title={t("Бюджет доходов и расходов", "Бюджет доходов и расходов")} style={{ cursor: 'pointer', display: 'flex', color: 'var(--text-muted)' }}>
                <BarChart3 size={13} />
              </span>
            </div>
          </div>
              ...
              return (
                <tr key={b.id} onClick={() => onSelect(b)}
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <button
                      onClick={e => { e.stopPropagation(); confirmDialog({ message: `Удалить бюджет «${b.name}»?`, danger: true }).then(ok => ok && deleteBdrBudget(b.id)); }}
