...
import { Search, ChevronDown, Plus, Trash2, X, Filter, BarChart3 } from 'lucide-react';
import { useFinanceStore, type BdrBudget } from '../financeStore';
import { APP_CURRENCY } from '../config/currency';

export type { BdrBudget };
...
interface Props { onSelect: (b: BdrBudget) => void; }

// --- Новый бюджет: модалка ---
const NewBudgetModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const { addBdrBudget, projects, budgetScenarios } = useFinanceStore();
  const [form, setForm] = useState({
    name: '', type: 'БДР' as 'БДР' | 'БДДС',
    currency: APP_CURRENCY, projectId: '', entityId: '',
    ...
    <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.4)',backdropFilter:'blur(4px)'}}>
      <div style={{background:'var(--bg-surface)',borderRadius:12,width:480,boxShadow:'0 20px 60px rgba(0,0,0,0.3)',border:'1px solid var(--border-subtle)'}}>
        ...
        <div style={{padding:'20px 24px 16px',borderBottom:'1px solid var(--border-subtle)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <h2 style={{margin:0,fontSize:16,fontWeight:700,color:'var(--text-primary)'}}>Новый бюджет доходов и расходов</h2>
        ...
        <form onSubmit={handleSubmit} style={{padding:'20px 24px',display:'flex',flexDirection:'column',gap:14}}>
          <div style={row}>
            ...
            <input style={inp} placeholder="Например: БДР 2026 Основной" value={form.name} onChange={e=>upd('name',e.target.value)} required/>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
            <div style={row}>
              <span style={label}>Тип</span>
              <select style={{...inp,appearance:'none',paddingRight:28}} value={form.type} onChange={e=>upd('type',e.target.value as 'БДР'|'БДДС')}>
                <option value="БДР">БДР</option>
                <option value="БДДС">БДДС</option>
              </select>
            </div>
            <div style={row}>
              <span style={label}>Валюта</span>
              <select style={{...inp,appearance:'none'}} value={form.currency} onChange={e=>upd('currency',e.target.value)}>
                <option value={APP_CURRENCY}>UZS сум</option>
          ...
          <div style={row}>
            <span style={label}>Сценарий</span>
...
export const BudgetBdrList: React.FC<Props> = ({ onSelect }) => {
  const { projects, bdrBudgets, deleteBdrBudget } = useFinanceStore();
  ...
  const filtered = bdrBudgets.filter(b => {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: 'var(--bg-base)' }}>
      {showModal && <NewBudgetModal onClose={() => setShowModal(false)} />}
      ...
      <div style={{ width: 240, flexShrink: 0, borderRight: '1px solid var(--border-subtle)', background: 'var(--bg-surface)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, padding: 16, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', marginBottom: 8 }}>Юрлицо</div>
          ...
          <div>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--text-muted)', marginBottom: 8 }}>Проект</div>
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div className="finance-page-header" style={{ height: 44, padding: '0 24px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: 'var(--bg-surface)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', letterSpacing: -0.01 }}>БДР</span>
              <span title="Бюджет доходов и расходов" style={{ cursor: 'pointer', display: 'flex', color: 'var(--text-muted)' }}>
                <BarChart3 size={13} />
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button onClick={() => setShowModal(true)} style={{ background: 'var(--color-primary)', color: 'var(--text-primary)', border: 'none', padding: '0 14px', height: 28, borderRadius: 6, fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', boxShadow: '0 2px 8px rgba(99, 102, 241, 0.25)' }}>
              <Plus size={13} /> Новый бюджет
            </button>
          </div>
        </div>
      ...
      <div className="finance-page-content finance-mobile-table" style={{ flex: 1, padding: '24px 32px', overflowY: 'auto' }}>
              ...
              return (
                <tr key={b.id} onClick={() => onSelect(b)}
                  ...
                  <td style={{ padding: '12px 14px', fontSize: 12 }}>
                    <span style={{padding:'2px 8px',borderRadius:4,fontSize:11,fontWeight:500,background:b.type==='БДР'?'rgba(99,102,241,0.1)':'rgba(16,185,129,0.1)',color:b.type==='БДР'?'#818cf8':'#10b981'}}>
                      {b.type}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <button
                      onClick={e => { e.stopPropagation(); if (confirm(`Удалить бюджет «${b.name}»?`)) deleteBdrBudget(b.id); }}
        ...
        {filtered.length === 0 && (
          <div style={{ padding: '60px 0', textAlign: 'center', color: 'var(--text-muted)', fontSize: 14 }}>
            {bdrBudgets.length === 0 ? 'Бюджеты ещё не созданы — нажмите «+ Новый бюджет»' : 'Бюджеты не найдены'}
          </div>
        )}
      </div>
      </div>
